...
async def fake_serial_task(context, data_file, update_delay=0.11):
    def loc_data(row): return LocationData(
        float(row['x']), float(row['y']), 0, float(row['quality']))

    def imu_data(row): return IMUData(Rotation(float(row["yaw"]), 0, 0),
                                      Transform(float(row["a_r_x"]), float(
                                          row["a_r_y"]), 0),
    ...
    try:
        ...
        if serial_data_file is None:
            location_task = asyncio.create_task(
                serial_man(context))  # use real serial
        else:
            location_task = asyncio.create_task(fake_serial_task(
                context, serial_data_file))  # get data from file
        ...
        if not disable_motor:
            ...
            context.from_web_queue.put(message)
